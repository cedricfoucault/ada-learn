...
jsPsych.plugins['pavlovia'] = (function() {
	 ...
	 * @private
	 */
	// == CEDRIC FOUCAULT'S EDIT BEGIN ==
	// - Add a callback parameter to call back for saving incomplete results
	const _init = async function(trial, configURL = 'config.json',
		saveIncompleteResultsCallback)
	// == CEDRIC FOUCAULT'S EDIT END ==	
	{
		try {
			// configure:
			...
			window.addEventListener('unload', (event) =>
			{
				if (_config.session.status === 'OPEN')
				{
					...
					{
						// == CEDRIC FOUCAULT'S EDIT BEGIN ==
						// - Skip _save(trial) if no trial was provided
						// - Call back the given the saveIncompleteResultsCallback if provided
						if (trial) {
							const data = jsPsych.data.get().csv();
	...
	const _finish = async function(trial, data)
	{
		try
		{
			...
			// == CEDRIC FOUCAULT'S EDIT BEGIN ==
	    // Skip _save if no trial or data was provided
			let response;
			if (trial && data) {
				response = await _save(trial, data, false);
	 ...
	 */
	// == CEDRIC FOUCAULT'S EDIT BEGIN ==
	// - Add a filename parameter to use for storing the data, rather than the default one.
	const _save = async function(trial, data, sync = false, filename)
	// == CEDRIC FOUCAULT'S EDIT END ==
	{
		// == CEDRIC FOUCAULT'S EDIT BEGIN ==
		let key = filename;
		if (!key) {
			// original code for initializing key
			const date = new Date();
			...
			key = _config.experiment.name + '_' + trial.participantId + '_' + 'SESSION' + '_' + dateString + '.csv';
		}
		// == CEDRIC FOUCAULT'S EDIT END ==
	...
	// == CEDRIC FOUCAULT'S EDIT BEGIN ==
	// Add private functions to the plugin object for external use
	plugin._init = _init;
	plugin._finish = _finish;
	plugin._save = _save;
	plugin._getConfig = () => { return _config; }
	// == CEDRIC FOUCAULT'S EDIT END ==
